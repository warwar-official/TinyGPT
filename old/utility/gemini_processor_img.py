...
class ApiKeyManager:
    ...
    def get_key(self, model_id: str):
        ...
        # Модель gemini-1.5-pro-latest підтримує зображення, тому перевіряємо її
        model_to_check = "gemini-1.5" if "1.5" in model_id else model_id
        if model_to_check not in self.MODEL_LIMITS:
        ...
        for i, key_info in enumerate(self.keys):
            ...
            if not is_blocked and key_info['model_limits'].get(model_to_check, 0) > 0:
                logging.info(f"Використовується ключ #{i+1} для моделі '{model_id}' (залишилось запитів: {key_info['model_limits'][model_to_check]}).")
                return key_info
        return None
    ...
    def decrement_limit(self, key_str: str, model_id: str):
        ...
        for key_info in self.keys:
            if key_info['key'] == key_str:
                if model_to_check in key_info['model_limits']:
                    key_info['model_limits'][model_to_check] -= 1
                break
        self._save_state()
...
class GeminiProcessor:
    ...
    Підтримує текстові та мультимодальні (текст + зображення) запити.
    """
    ...
    DEFAULT_MODEL_ID = "gemini-2.5" # Змінено на модель, що підтримує зображення
    ...
    def _make_api_request(self, prompt_text: str, model_id: str, images: list = None):
        """
        Виконує один запит до API. Може бути текстовим або мультимодальним.
        :param prompt_text: Текстова частина запиту.
        :param model_id: Ідентифікатор моделі для запиту.
        :param images: Список шляхів до локальних зображень.
        """
        ...
        # Побудова тіла запиту (може містити текст та зображення)
        parts = [{"text": prompt_text}]
        if images:
            if "1.5" not in model_id: # Перевірка, чи модель підтримує зображення
                 logging.warning(f"Модель '{model_id}' може не підтримувати зображення. Спроба відправки...")
            for image_path in images:
                try:
                    # Визначення MIME типу та кодування зображення в base64
                    mime_type, _ = mimetypes.guess_type(image_path)
                    if not mime_type or not mime_type.startswith('image'):
                        logging.warning(f"Не вдалося визначити тип зображення для '{image_path}'. Пропуск файлу.")
                        continue
                    ...
                    logging.info(f"Зображення '{image_path}' додано до запиту.")
                except FileNotFoundError:
                    logging.error(f"Файл зображення не знайдено: '{image_path}'. Пропуск.")
                except Exception as e:
                    logging.error(f"Не вдалося обробити зображення '{image_path}': {e}")
            ...
            try:
                with request.urlopen(req) as response:
                    if response.status == 200:
                        ...
                        try:
                            # Перевірка наявності контенту у відповіді
                            if 'candidates' in response_body and response_body['candidates']:
                                content = response_body['candidates'][0]['content']['parts'][0]['text']
                                return content, False
                            else:
                                # Обробка випадків, коли відповідь порожня (наприклад, через фільтри безпеки)
                                logging.warning(f"API повернув успішний статус, але відповідь не містить контенту. Деталі: {response_body}")
                                return "Відповідь не була згенерована, можливо через спрацювання фільтрів безпеки.", False
                ...
                if e.code == 429:
                    ...
                    try:
                        ...
                        logging.error(f"Отримано помилку від API: {error_details}")
                    except Exception as read_error:
                         logging.error(f"Не вдалося прочитати тіло помилки: {read_error}")
                    return None, False
    ...
    def run(self):
        ...
        for i, prompt_info in enumerate(self.prompts):
            ...
            # Використовуємо модель з промту, або Pro якщо є зображення, або стандартну
            #default_model = "gemini-1.5-pro" if prompt_info.get('images') else self.DEFAULT_MODEL_ID
            model_id = prompt_info.get('model', self.DEFAULT_MODEL_ID)
            images = prompt_info.get('images', []) # Отримуємо список зображень

            if not keywords_groups:
                if not placeholders:
                    self._handle_message("Ключові слова не надані, шаблон без плейсхолдерів. Виконується один запит.")
                    result, _ = self._make_api_request(template, model_id, images=images)
            ...
            for j, keywords in enumerate(keywords_groups):
                ...
                result, _ = self._make_api_request(final_prompt, model_id, images=images)
                if result: self._write_result(output_file, result)
